...
grid.rf <- expand.grid(mtry = 2^(1:5))
...
pred.rf <- predict(mdl.rf, newdata = ds.validation)
...
mdl.c50 <- train(classe ~ ., data = ds.training, method = "C5.0", trControl = tr_control, tuneGrid = grid.c50)
...
ds.combined <- data.frame(classe = ds.validation.tr$classe,
                          rf = predict(mdl.rf, newdata = ds.validation.tr),
                          svm = predict(mdl.svm, newdata = ds.validation.tr),
                          c50 = predict(mdl.c50, newdata = ds.validation.tr))

# random forest, 6 fold cross-validation
set.seed(42)
#mdl.final <- randomForest(classe ~ ., ds.validation.tr)
mdl.filan <- train(classe ~ ., ds.validation.tr, method = "rf", trControl = tr_control)
pred.final <- predict(mdl.final, newdata = ds.validation.ts)

# run model on testing set
ds.ts <- ds_raw.ts[,c("problem_id", pred_cols)]
ds.testing <- predict(ds.tr.pca, ds.ts[,-(2:8)])
pred.results <- predict(mdl.final, newdata = ds.testing[,-1])
